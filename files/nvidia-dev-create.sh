...
if modprobe nvidia NVreg_EnableMSI=0
then
  NVIDIA_3D_CONTROLLER=$(
    echo "$NVIDIA_DEVICES" | grep -c "3D controller"
  )

  NVIDIA_VGA_CONTROLLER=$(
    echo "$NVIDIA_DEVICES" | grep -c "VGA compatible controller"
  )
  ...
  for i in $(seq 0 $NVIDIA_N_DEVICES)
  do
  ...
  mknod \
    ...
    c \
    "$NVIDIA_DEVICE_ID" \
    "$NVIDIA_CONTROLLER_DEVICE_NUMBER"
else
  echo "could not successfully load nvidia module" >&2
  exit 1
fi

if modprobe nvidia_uvm
then
  NVIDIA_UVM_DEVICE_ID=$(
    awk '$2 == "nvidia-uvm" { print $1 }' /proc/devices
  )
